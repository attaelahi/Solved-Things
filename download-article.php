<?php
  ...
  if($row > 0){
    ...
    while($data=mysqli_fetch_assoc($res)){
      ...
      <article>
        <h1>'.$article_title.'</h1>
        <div> 
          <p>Solved Things.<br><br>'.date("d M Y",$article_date).'</p>
        </div>
        <div>
          <p>
            '.nl2br($article_desc).'
  ...
  $mpdf->SetWatermarkText('SolvedThings');

  // Specifying the font to use for watermark text
  $mpdf->watermark_font = 'SolvedThings'; 
  ...
  $file = 'SolvedThings-File'.$article_id.'-DT'.time().'.pdf';
